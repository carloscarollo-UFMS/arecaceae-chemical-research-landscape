get_script_path <- function() {
  ...
  fallback <- file.path(
    getwd(), "Supplementary_Code_S1_model_research_selection.R"
  )
  if (file.exists(fallback)) return(normalizePath(fallback, winslash = "/"))
  stop(
    "Unable to determine the script path. Run the file with Rscript or ",
    "RStudio, or define ARECACEAE_SCRIPT_PATH.",
    call. = FALSE
  )
}
...
if (!capabilities("cairo")) {
  stop("The installed R build does not support Cairo graphics.", call. = FALSE)
}
...
first_existing <- function(paths, label) {
  ...
  if (length(found) == 0L) {
    stop(
      label, " was not found. Paths examined:\n- ",
...
SPECIES_FILE <- first_existing(
  ...
  "Integrated species table with GBIF and GHS data"
)
...
if (!script_copy_ok) {
  stop("The analysis script could not be copied to the output directory.", call. = FALSE)
}
...
message("[01/10] Reading and auditing the integrated dataset...")
...
if (length(missing_columns) > 0L) {
  stop(
    "The integrated dataset is missing the following columns: ",
...
if (nrow(species) != 2582L || uniqueN(species$WCVP_taxon_id) != 2582L) {
  stop("Expected 2,582 unique WCVP taxon units.", call. = FALSE)
}
if (sum(species$Research_studied_binary == 1L, na.rm = TRUE) != 173L) {
  stop("Expected 173 studied species.", call. = FALSE)
}
if (!inherits(species_tree, "phylo") || ape::Ntip(species_tree) != 2233L) {
  stop("Expected a pruned WCVP phylogeny with 2,233 tips.", call. = FALSE)
}
...
message("[02/10] Constructing functional axes and standardised predictors...")
...
message("[03/10] Summarising taxonomic and geographical coverage...")
...
message("[04/10] Fitting the main logistic and phylogenetic models...")
...
message("[05/10] Fitting trait-specific models...")
...
message("[06/10] Fitting bias-reduced human-use and economic models...")
...
message("[07/10] Constructing diagnostics and quality-control checks...")
...
if (any(qa$Status == "FAIL")) {
  ...
  stop(
    "Stage 05B quality-control checks failed. See Table_QA_05B.csv.",
    call. = FALSE
  )
}

message("[08/10] Writing processed data, tables, and model objects...")
...
message("[09/10] Generating vector figures...")
...
message("[10/10] Writing the execution log and session information...")
...
log_lines <- c(
  "STAGE 05B — RESEARCH-SELECTION MODELS",
  "Version: 1.0.3",
  paste0("Started: ", format(started_at, "%Y-%m-%d %H:%M:%S")),
  paste0("Finished: ", format(finished_at, "%Y-%m-%d %H:%M:%S")),
  paste0("Duration_min: ", round(as.numeric(difftime(
    finished_at, started_at, units = "mins"
  )), 2)),
  paste0("WCVP_taxon_units: ", nrow(species)),
  paste0("Studied_species: ", sum(species$Research_studied_binary == 1L)),
  ...
  paste0("QA_failures: ", qa[Status == "FAIL", .N]),
  "Conclusion: research-selection correlates; associations are not causal"
)
writeLines(
  log_lines,
